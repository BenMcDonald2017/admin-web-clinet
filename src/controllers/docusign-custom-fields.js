...
export const getDocuSignCustomFieldData = (data) => {
  ...
  // [`${type}_checkbox_is_daughter`]:             isPerson && /^female$/i.test(gender) && /^(?:child|daughter)/i.test(relation),
  // [`${type}_checkbox_is_son`]:                  isPerson && /^male$/i.test(gender) && /^(?:child|son)/i.test(relation),
  // [`${type}_checkbox_is_domestic_partner`]:     isPerson && /^domestic\s*partner$/i.test(relation),
  // [`${type}_checkbox_is_married`]:              isPerson && /^married$/i.test(status),
  // // check 'single' if person exists, and isn't 'married', in a 'domestic partner[ship]', or otherwise has a status of 'single'
  // [`${type}_checkbox_is_single`]:               isPerson && (!/^(?:married|domestic\s*partner)$/i.test(status) || /^single$/i.test(status)),
  // [`${type}_gender_checkbox_female`]:           isPerson && /^female$/i.test(gender),
  // [`${type}_gender_checkbox_male`]:             isPerson && /^male$/i.test(gender),
  ...
  Object.assign(
    formFieldData,
    fetchAndFillDataFor(worker, 'worker').data,
    fetchAndFillDataFor(spouse, 'spouse').data,
    ...iterate(6, i => fetchAndFillDataFor(getFamilyMember(i), `dep${i + 1}`).data),
  )

  return formFieldData
}

function fetchAndFillDataFor(person = {}, label = '', type = 'text') {
